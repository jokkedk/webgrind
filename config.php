...
class Webgrind_Config extends Webgrind_MasterConfig {
     ...
     */
    static $pythonExecutable = '/usr/local/bin/python';
    ...
    static function xdebugOutputFormat() {
        ...
        if ($outputName == '') return '/^cachegrind\.out\..+$/';

        return '/^'.preg_replace('/(%[^%])+/', '.+', $outputName).'$/';
    }
    ...
    static function xdebugOutputDir() {
        ...
        if ($dir == '') return realpath(Webgrind_Config::$profilerDir).'/'; // Ini value not defined
    ...
    static function storageDir() {
        if (!empty(Webgrind_Config::$storageDir)) return realpath(Webgrind_Config::$storageDir).'/';

        if (!function_exists('sys_get_temp_dir') || !is_writable(sys_get_temp_dir())) { // use xdebug setting
