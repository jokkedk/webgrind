...
if (ini_get('date.timezone') == '') date_default_timezone_set(Webgrind_Config::$defaultTimezone);
...
try {
    switch (get('op')) {
        ...
        case 'function_list':
            ...
            foreach ($functions as $function) {                                                                                                           $remainingCost -= $function['summedSelfCost'];
        ...
        case 'fileviewer':
            ...
            $code = str_replace(array('<code>', '</code>', '<br />', '&nbsp;&nbsp;&nbsp;&nbsp;', '&nbsp;'), array('', '', '', "\t", ' '), $code);

            $lines_cnt = count_chars($code, 1);
            $lines_cnt = $lines_cnt[13]+1;

            $lines = range(1, $lines_cnt);
            $lines[$line-1] = '<div class="num" id="line_emph"><a name="line_emph">'.$line.'</a></div>'.$lines[$line];
            unset($lines[$line]);
            $lines = implode("\n", $lines);
        ...
        case 'function_graph':
            ...
            if (!file_exists($filename)) {
                shell_exec(
                    Webgrind_Config::$pythonExecutable.' library/gprof2dot.py -n '.$showFraction.' -f callgrind '.Webgrind_Config::xdebugOutputDir().$dataFile
            ...
            break;
        case 'version_info':
            echo @file_get_contents('http://jokke.dk/webgrindupdate.json?version='.Webgrind_Config::$webgrindVersion);
            break;
        default:
            $welcome = '';
