...
class Webgrind_FileHandler {
    ...
    /**
     * @return self Singleton instance of the filehandler
     */
    ...
    private function getInvokeUrl($file) {
        if (preg_match('/.webgrind$/', $file)) return 'Webgrind internal';
    ...
    private function getFiles($format, $dir) {
        ...
        foreach ($list as $file) {
            ...
            if (false !== strstr($absoluteFilename, Webgrind_Config::$preprocessedSuffix)) continue;

            // Make sure that script never parses the profile currently being generated. (infinite loop)
            if ($selfFile == realpath($absoluteFilename)) continue;
    ...
    private function getPrepFiles($format, $dir) {
        ...
        $test_path = (function_exists('xdebug_get_profiler_filename') ? realpath(xdebug_get_profiler_filename()) : false);

        foreach ($list as $file) {
            $absoluteFilename = $dir.$file;

            // Make sure that script does not include the profile currently being generated. (infinite loop)
            if ($test_path && $test_path == realpath($absoluteFilename)) continue;
     ...
     * @param string $costFormat Cost format for the reader
    ...
    public function getTraceReader($file, $costFormat) {
        ...
        try {
            if (!is_file($prepFile)) Webgrind_Preprocessor::parse(Webgrind_Config::xdebugOutputDir().$file, $prepFile);
